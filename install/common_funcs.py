...
def get_mysql_credentials(host=None, port=None, user=None, password=None, database=None, no_confirm=False):
    ...
    host = os.environ.get('DOCKER_FLASK_MANAGER_DB_HOST',host)
    while host is None:
    ...
    port = os.environ.get('DOCKER_FLASK_MANAGER_DB_PORT',port)
    while port is None:
        try:
    ...
    user = os.environ.get('DOCKER_FLASK_MANAGER_DB_USER',user)
    if user is None:
        user = input("Enter your mysql user: ")
    password = os.environ.get('LYCHEE_CRM_DB_PASSWORD',password)
    if password is None:
        password = input("Enter your mysql password:")
    database = os.environ.get('DOCKER_FLASK_MANAGER_DB_DATABASE',database)
    if database is None:
